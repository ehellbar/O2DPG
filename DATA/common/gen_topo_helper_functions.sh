...
if [[ $EPNSYNCMODE == 1 ]] && workflow_has_parameter QC; then
...
add_QC_from_consul()
{
  [[ $EPNSYNCMODE == 1 ]] || { echo "Error fetching QC JSON $1: consul server only set for EPNSYNCMODE == 1 " 1>&2 && exit 1; }
...
add_QC_from_apricot()
{
  [[ $EPNSYNCMODE == 1 ]] || { echo "Error fetching QC JSON $1: apricot server only set for EPNSYNCMODE == 1 " 1>&2 && exit 1; }
  if [[ ! -z ${GEN_TOPO_QC_JSON_FILE:-} ]]; then
    if [[ ${1} =~ "?" ]]; then
