...
add_QC_JSON() {
  if [[ ${2} =~ ^consul://.* ]]; then
    [[ $EPNSYNCMODE == 1 ]] ||  { echo "Error fetching QC JSON $2: consul server only set for EPNSYNCMODE == 1 " 1>&2 && exit 1; }
    ...
    [[ $EPNSYNCMODE == 1 ]] || { echo "Error fetching QC JSON $2: apricot server only set for EPNSYNCMODE == 1 " 1>&2 && exit 1; }
    TMP_FILENAME=$FETCHTMPDIR/$1.$RANDOM.$RANDOM.json
	  if [[ ${2} =~ "?" ]]; then
